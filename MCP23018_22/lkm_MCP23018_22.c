...
MODULE_PARM_DESC(nRST_pin, " MCP23018 reset pin (default=6)");     ///< parameter description

static unsigned int irqA_pin = 5u; 	///< Default
module_param(irqA_pin, uint, S_IRUGO); ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(irqA_pin, " MCP23018 interrupt A pin (default=5)");     ///< parameter description

static unsigned int irqB_pin = 13u; 	///< Default
module_param(irqB_pin, uint, S_IRUGO); ///< Param desc. S_IRUGO can be read/not changed
MODULE_PARM_DESC(irqB_pin, " MCP23018 interrupt B pin (default=13)");     ///< parameter description
...
#define DEVICE_NAME "MCP23018_22_I2C_DRV"
...
static int __init MCP23018_init(void){
	...
	int ret = 0;
	int i = 0;
	printk(KERN_INFO "MCP23018: Initializing the MCP23018 (0x22) LKM.\n");
...
#endif
			aresult[i] = request_threaded_irq(irqNumber[i], (irq_handler_t)(MCP23018_irq_handler[i]), (irq_handler_t)(MCP23018_threaded_fn[i]), /* irq flags */ IRQF_TRIGGER_FALLING, /* default INT setting on MCP23018 */ "MCP23018_22_IRQ", /* Used in /proc/interrupts to identify the owner */ NULL /* The *dev_id for shared interrupt lines, NULL is okay */);
...
static irq_handler_t MCP23018_IRQA_threaded_fn(unsigned int irq, void *dev_id, struct pt_regs *regs) {
	...
	unsigned int bytes_read = 0u;
	bytes_read = i2c_get_reg_data(GPIOA, 1, buffer);
	if (bytes_read > 0) {
		u32INTCAP &= ~(0xFFu);
...
static irq_handler_t MCP23018_IRQB_threaded_fn(unsigned int irq, void *dev_id, struct pt_regs *regs) {
	...
	bytes_read = i2c_get_reg_data(GPIOB, 1, buffer);
	if (bytes_read > 0) {
		u32INTCAP &= ~(0xFF00u);
...
static int read_button_matrix(void *arg) {
	...
	while (!kthread_should_stop()) {
		...
		//	printk(KERN_INFO "INTCAPA = 0x%02x.", u8PortA[0]);
			...
			printk(KERN_INFO "Error reading butt. matrix at PORTA.");
		}
