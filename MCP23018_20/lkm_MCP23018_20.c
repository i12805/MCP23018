...
static unsigned int nRST_pin = 27u; 	///< Default
...
static unsigned int irqA_pin = 17u; 	///< Default
...
static unsigned int irqB_pin = 22u; 	///< Default
...
static unsigned int gpioIRQ[IRQ_PIN_COUNT] = {17, 22};
...
#define DEVICE_NAME "MCP23018_20_I2C_DRV"
...
static int MCP23018_probe(struct i2c_client *client) {
	unsigned char buffer[DATA_PAGE_SIZE] = {0};
	unsigned int bytes_read = 0u;
	unsigned int i = 0u;
	unsigned char WRdata[2] = {0x01u, 0xFFu};

	printk(KERN_INFO "I2C client: Probing ...");

	/* config IOCON for clr irq on INCAP read */
	bytes_read = i2c_set_reg_data((int)IOCON, 1, WRdata);

	/* config irq */
	//WRdata[0] = 0xffu;

	/* set irq detection to be based on DEFVAL */
	//bytes_read = i2c_set_reg_data((int)INTCONA, 2, WRdata);
	
	/* set default value for identifying irq */
	//bytes_read = i2c_set_reg_data((int)DEFVALA, 2, WRdata);
	
	bytes_read = i2c_set_reg_data((int)GPINTENB, 1, WRdata);

	/* read back all registers */
	if (i2c_get_reg_data(0x00, 22, buffer) < 0) {
		printk(KERN_INFO "Error reading regs.");
	} else {
		printk(KERN_INFO "Read back all registers:");
		for (i = 0; i < 22; i++) printk(KERN_INFO "\t0x%02x = 0x%02x;", i, buffer[i]);
	}
...
static int __init MCP23018_init(void){
	...
	int ret = 0;
	int i = 0;
	printk(KERN_INFO "MCP23018_20: Initializing the MCP23018 LKM.\n");
	...
	for(i = 0; i < IRQ_PIN_COUNT; i++) {
		...
		gpio_set_debounce(gpioIRQ[i], 200);      // Debounce the button with a delay of 200ms
...
#endif
			aresult[i] = request_threaded_irq(irqNumber[i], (irq_handler_t)(MCP23018_irq_handler[i]), (irq_handler_t)(MCP23018_threaded_fn[i]), /* irq flags */ IRQF_TRIGGER_FALLING, /* default INT setting on MCP23018 */ "MCP23018_20_IRQ", /* Used in /proc/interrupts to identify the owner */ NULL /* The *dev_id for shared interrupt lines, NULL is okay */);
	...
	MCP23018_kobj = kobject_create_and_add("MCP23018_20", kernel_kobj->parent); // kernel_kobj points to /sys/kernel
	if(!MCP23018_kobj){
...
CLEAN_I2C:
	printk(KERN_INFO "Release MCP32018 I2C driver.");
	i2c_driver_exit();
CLEAN_GPIO:
...
static void __exit MCP23018_exit(void){
	...
	printk(KERN_INFO "MCP23018_20: Goodbye from the LKM!\n");
}
